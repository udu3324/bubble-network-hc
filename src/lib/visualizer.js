...
export function tick(delta) {
    ...
    if (panOriginX != null) {
        ...
        targetX = prevCamX - (panX - panOriginX) / camera.zoom
        targetY = prevCamY - (panY - panOriginY) / camera.zoom
        let dx = targetX - camera.x
        let dy = targetY - camera.y

        camera.x += dx / cameraEase
        camera.y += dy / cameraEase
    } else {
        if (prevCamX != null) {
            prevCamX = null
        }
    ...
    if (zoomToKing) {
        setResetMode(false)
        camera.zoom += (0.2 - camera.zoom) / 30
        let tX = nodes[king].pos.x
        let tY = nodes[king].pos.y

        camera.x += (tX - camera.x) / 10
        camera.y += (tY - camera.y) / 10
        targetX = camera.x
        targetY = camera.y
    }

    if (resetMode) {
        camera.zoom += (0.05 - camera.zoom) / 30
...
class Node { // id correlates to id in the list of peoples
    ...
    inRange() {
        ...
        if (posX(this.pos.x) <= canvas.width + this.rad * camera.zoom * 4 && posX(this.pos.x) >= 0 - this.rad * camera.zoom * 4) {
            if (posY(this.pos.y) <= canvas.height + this.rad * camera.zoom * 4 && posY(this.pos.y) >= 0 - this.rad * camera.zoom * 4) {
                return true
            }
        }
        return false
    ...
    inZoom() { // zooming out prioritizes nodes w/ larger connections
        return Math.pow(this.connectionLines.length, 2) > Math.pow((100-visibility), 2) / camera.zoom
    }
    showName() {
